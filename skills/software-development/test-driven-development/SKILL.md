...
If the project does not already use pytest, fall back to the standard library runner with explicit discovery so you still get a real pass/fail signal:

```bash
python -m unittest discover -s tests -p 'test*.py' -v
```

A plain `python -m unittest -v` can report `Ran 0 tests` even when tests exist, so prefer discovery when validating a new or unfamiliar repo.

See `references/unittest-discovery.md` for the exact command pattern and the failure mode we observed in a fresh repo.
